...
function generateClipV2(complexFilter:Array<Record<any, any>>, clipName:string, start:string, end:string): Promise<any> {
  // TODO : 
  // Manage extension ! for input/output
  return new Promise((resolve, reject) => {
    ffmpeg(path.resolve(process.cwd(), "fixtures", "Se lever tôt ne te rendra pas meilleur (et c'est tant mieux).webm"))
...
describe('AppController', () => {
  ...
  it("step4.consumer.v2", async () => {
    ...
    for ( let clip in fixtureCompletion ) {
      fixtureCompletion[clip] = fixtureCompletion[clip].map((seg) => {
  ...
  // it('xd', async() => {
  //   let text = "C'est vrai que chaque député m'a demandé ça. Ça part pas en une minute en fait. « Est-ce qu'il faut se lever tôt pour être productif ?";
  //   let doc = nlp(text)
